...
import { type CompassServiceConfig, loadConfig } from "./config.js";
import { CompassStore } from "./store/compass-store.js";
import { LineageStore } from "./store/lineage-store.js";
...
export interface CreateServerOptions {
  ...
  config?: CompassServiceConfig;
  /** Inject an existing store (useful for tests). */
  store?: CompassStore;
  /** Inject an existing lineage store (useful for tests). */
  lineageStore?: LineageStore;
}

/**
 * Build and configure the Fastify server instance for the Compass service.
 */
export async function createServer(
  options: CreateServerOptions = {},
): Promise<FastifyInstance> {
  const config = options.config ?? loadConfig();
  const store = options.store ?? new CompassStore();
  ...
  await app.register(v2Routes, { prefix: "/api/v2", store, lineageStore });
