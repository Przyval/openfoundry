 ...
 *
 * CRITICAL: These tests verify that multi-tenancy via Postgres RLS works
 * correctly — Tenant A cannot see or modify Tenant B's data.
 *
...
describe.skipIf(!GATEWAY_UP)("Tenant Isolation (Multi-Tenancy RLS)", () => {
  ...
  // -------------------------------------------------------------------------
  it("Tenant A creates an ontology; Tenant B cannot list it", async () => {
    ...
    const foundByB = list.data?.find(
      (o: { rid: string }) => o.rid === tenantAOntologyRid,
    );
  ...
  // -------------------------------------------------------------------------
  it("Tenant B cannot GET Tenant A's ontology by RID", async () => {
    const res = await api(
      "GET",
      `/api/v2/ontologies/${tenantAOntologyRid}`,
      tenantBToken,
    );
  ...
  // -------------------------------------------------------------------------
  it("Duplicate signup username returns 409 Conflict", async () => {
    const suffix = Date.now().toString(36);
